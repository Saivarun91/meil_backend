...
# Helper function to format attributes values for short_name (values only, no attribute names)
def format_attributes_for_short_name(attributes):
    """Format attributes dictionary as 'value, value' string (no attribute name labels)"""
    if not attributes or not isinstance(attributes, dict):
        return ""
    pairs = [str(v) for k, v in attributes.items() if v]  # Only values, no keys
    return ", ".join(pairs)
...
def create_itemmaster(request):
    ...
    try:
        ...
        sap_item_id = int(raw_sap_id) if raw_sap_id not in (None, "", "null") else None
