    ...
    try:
        ...
        for attr in attributes:
            ...
            if print_priority:
                ...
                if duplicate:
                    return JsonResponse({
                        "error": f"Priority {print_priority} is already assigned to attribute '{duplicate.attribute_name}' in this material group"
                    }, status=400)
...
def update_matgattribute(request, item_id):
    ...
    try:
        ...
        if "print_priority" in data:
            new_priority = data["print_priority"]
            if new_priority:
                duplicate = MatgAttributeItem.objects.filter(
                ...
                if duplicate:
                    return JsonResponse({
                        "error": f"Priority {new_priority} is already assigned to attribute '{duplicate.attribute_name}' in this material group"
                    }, status=400)
